    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                if node.name == "is_triangle":
                    find_func = node.name

        if not find_func:
            raise ValueError("ОШИБКА: Не найдена функция 'is_triangle(x)'")
        ...
        try:
            res = test_7_2_2_1(path_tmp_file)
            result.append(res)
...
def test_7_2_2_1(path_tmp_file: str):
    ...
    try:
        for i in range(len(test_input)):
            ...
            is_triangle = getattr(user_module, "is_triangle")
            # Вызываем функцию с тестовым вводом
            out_answer = is_triangle(*test_input[i])
