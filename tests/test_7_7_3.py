...
def test_7_7_3(path_tmp_file: str, task_num_test: str):
    ...
    try:
        ...
        for item, expected_type in attr_search.items():
            ...
            if item in attr_search:
                ...
                # ДОПОЛНЕНИЕ: Проверка рекурсивности функции get_rec_sum
                if item == 'get_rec_sum' and attr_type == 'function':
                    ...
                    for node in ast.walk(tree):
                        if (
                            ...
                            and node.func.id == 'get_rec_sum'
                        ):
                            is_recursive = True
                            break

                    if is_recursive:
                        result.append(f"Функция 'get_rec_sum': recursive")
                    else:
        ...
        try:
            res = test_7_7_3_1(path_tmp_file, task_num_test)
            result.append(res)
...
def test_7_7_3_1(path_tmp_file: str, task_num_test: str):
    ...
    try:
        for i in range(len(test_input)):
            ...
            # Получаем функцию get_rec_sum из модуля пользователя
            user_func = getattr(user_module, 'get_rec_sum')
            # Выполняем функцию
            output = user_func(test_input[i])
