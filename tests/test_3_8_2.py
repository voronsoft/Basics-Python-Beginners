...
def test_3_8_2(path_tmp_file: str, task_num_test: str):
    ...
    # Проверяем, есть ли в коде print(*lst
    ...
    if "print(*lst" not in user_code:
        raise ValueError(
            "------------- FAIL Тест -------------\n"
            "В коде не найдено: print(*lst\nЯ проверил ваш код, не хитрите )))"
        )
    if any(i in user_code for i in ("if ", "else ", "elif ")):
        raise ValueError(
            "------------- FAIL Тест -------------\n" "Не используйте условные операторы в решении задачи!"
        )
