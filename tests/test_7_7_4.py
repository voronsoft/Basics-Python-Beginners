...
def test_7_7_4(path_tmp_file: str, task_num_test: str):
    ...
    try:
        ...
        attr_search = {"fib_rec": "function", "N": "int"}
        ...
        for item, expected_type in attr_search.items():
            ...
            if item in attr_search:
                ...
                # ДОПОЛНЕНИЕ: Проверка рекурсивности функции fib_rec
                if item == 'fib_rec' and attr_type == 'function':
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id == 'fib_rec':
                            is_recursive = True
                            break
        ...
        try:
            res = test_7_7_4_1(path_tmp_file, task_num_test)
            result.append(res)
...
def test_7_7_4_1(path_tmp_file: str, task_num_test: str):
    ...
    try:
        for i in range(len(test_input)):
            ...
            # Получаем функцию fib_rec из модуля пользователя
            user_func = getattr(user_module, 'fib_rec')
            # Выполняем функцию
