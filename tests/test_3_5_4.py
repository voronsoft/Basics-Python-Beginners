# 3_5_4 тест для задачи
import subprocess


def test_3_5_4(path_tmp_file: str, task_num_test: str):
    """Функция тестирования кода пользователя"""
    # Проверяем, есть ли в коде f строки (f", f', f''', f""")
    with open(path_tmp_file, "r", encoding="utf-8") as f:
        user_code = f.read()
        print(user_code)

    # Проверяем, содержит ли код хотя бы одну f-строку вида:
    f_string_prefixes = ('f"', "f'", 'f"""', "f'''")
    has_raw_string = any(prefix in user_code for prefix in f_string_prefixes)

    if not has_raw_string:
        raise ValueError(
            "------------- FAIL Тест -------------\n"
            "Вы не использовали f-строки в своем коде.\nЯ проверил ваш код )))"
        )
    ...
    # Ожидаемый результат
    expected_output = (
        "г. Лондон, ул. Рузвельт, д. 26, кв. 4",
        "г. Leon, ул. Garden, д. 10, кв. 5",
    )
