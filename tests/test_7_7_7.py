    ...
    try:
        ...
        for item, expected_type in attr_search.items():
            ...
            if item in attr_search:
                ...
                # ДОПОЛНЕНИЕ: Проверка рекурсивности функции get_path
                if item == 'get_path' and attr_type == 'function':
                    ...
                    if is_recursive:
                        result.append(f"Функция '{item}': recursive")
                    else:
        ...
        try:
            res = test_7_7_7_1(path_tmp_file, task_num_test)
            result.append(res)
