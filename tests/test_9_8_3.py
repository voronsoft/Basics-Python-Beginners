...
def test_9_8_3(path_tmp_file: str, task_num_test: str):
    ...
    try:
        ...
        for node in ast.walk(tree):
            # Поиск определения функции get_sum с аргументами
            if isinstance(node, ast.FunctionDef):
                if node.name == "get_sum" and len(node.args.args) == 1:
                    func_found = True
        ...
        if not func_found:
            raise RuntimeError("ОШИБКА: В коде не найдена функция get_sum(it)")

        result.append("Найдена функция get_sum(it)")
        ...
        # Функциональный тест
        try:
            res = test_9_8_3_1(path_tmp_file)
            result.append(res)
...
def test_9_8_3_1(path_tmp_file: str):
