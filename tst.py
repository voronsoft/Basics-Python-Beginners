...
def test_7_12_1_1(path_tmp_file: str, fnc_name, dec_name):
    """Функция тестирования пользовательского кода"""
    ...
    result = []  # Сюда будем сохранять результаты всех тестов
    ...
    for i in range(len(test_input)):
        ...
        # Создаём буферы для подмены
        input_buffer = StringIO(test_input[i])
        output_buffer = StringIO()
        error_buffer = StringIO()
        ...
        try:
            ...
            if captured_output == expected_output[i]:
                ...
                raise ValueError(
                    ...
                    f"Входные данные: {test_input[i]}\n"
                    f"Ожидалось: {expected_output[i]}\n"
                    f"но получено: {captured_output}\n"
                )

        except Exception as e:
            # Восстанавливаем потоки даже при ошибке
            sys.stdin = original_stdin
            sys.stdout = original_stdout
            sys.stderr = original_stderr

            # Получаем всё, что было выведено в stderr
            error_output = error_buffer.getvalue().strip()

            # Сохраняем результаты до момента ошибки и саму ошибку
            error_info = "\n".join(result)
            error_info += f"\nИСКЛЮЧЕНИЕ: {e}\n"
            if error_output:
                error_info += f"\nSTDERR:\n{error_output}"
            ...
            # Гарантируем восстановление потоков даже при успехе
            sys.stdin = original_stdin
            sys.stdout = original_stdout
            sys.stderr = original_stderr

    return "\n".join(result)  # Возвращаем полный отчёт по тестам